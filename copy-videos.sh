...
# Find all video.mov and video.MOV files in subdirectories and copy them to destination
find "$src_dir" -type f \( -name "video.mov" -o -name "video.MOV" \) -exec sh -c '
    ...
    for file; do
        ...
        # Remove previous directory with the pattern *-video*
